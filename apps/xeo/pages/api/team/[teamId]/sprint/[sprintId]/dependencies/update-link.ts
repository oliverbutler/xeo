...
import Joi from 'joi';
import {
  createLinkBetweenTickets,
  removeLinkBetweenTickets,
} from 'utils/notion/ticket';
...
export type DeleteRemoveNotionTicketLink = APIRequest<
  {
    sourceTicketId: string;
    targetTicketId: string;
  },
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    ...
    case 'DELETE':
      return await deleteHandler(req, res, sprint);
  }
...
const deleteHandler = async (
  ...
  try {
    ...
    return apiResponse<DeleteRemoveNotionTicketLink>(res, { success: true });
  } catch (e) {
    return apiError(
      res,
      {
